...
const getNewCursorLocation = (
  textStream: string,
  currentLine: number,
  currentCharacter: number
): { newCharacterLocation: number; newLineLocation: number } => {
  const numberOfNewLines = countNewLines(textStream);
  const newCharacterLocation =
    numberOfNewLines === 0 ? countCharacters(textStream) + currentCharacter : 0;
...
const getVisibleText = async (
  ...
  visibleRanges.forEach((range, index) => {
    if (
      lastVisibleRangeEnd &&
      document.offsetAt(range.start) - document.offsetAt(lastVisibleRangeEnd) >
        1
    ) {
      visibleText += "...";
    }
...
const streamCompletion = async (
  prompt: string,
  onDataFunction: (chunk: any) => void,
  cancelFunction: (source: any, reject: () => void) => void
): Promise<void> => {
  const openaikey = await workspace
    .getConfiguration()
    .get("wolverine.UNSAFE.OpenaiApiKeySetting");
  ...
  const configurationModel = await workspace
    .getConfiguration()
    .get("wolverine.model");
  const data = {
...
const useBufferToUpdateTextContentsWhileStreamingOpenAIResponse = async (
  ...
  streamCompletion(prompt, onDataFunction, cancelFunction).then(
    () => (doneStreaming = true)
  );
  ...
  if (startWithNewLine) {
    await activeEditor.edit((editBuilder) => {
      editBuilder.insert(new Position(currentLine, currentCharacter), "\n");
    });
  }
  while (!doneStreaming || buffer.length >= 0) {
    const word: string | undefined = buffer.shift();
    if (word) {
      let { newCharacterLocation, newLineLocation } = getNewCursorLocation(
        word,
        currentLine,
        currentCharacter
      );
...
const directedHealPrompt = async (
  text: string,
  filepath: string
): Promise<string> => {
...
`;
  const configurationInstructions = await workspace
    .getConfiguration()
    .get("wolverine.prompt");
  return `
ADDITIONAL CONTEXT:
filepath: ${filepath}
INSTRUCTIONS:
...
const directedReviewResponseHealPrompt = async (
  text: string,
  filepath: string,
  number: number
): Promise<string> => {
...
- Your entire output MUST be valid code.
- You may communicate back, but they MUST be in comments
- The code sent to you will have a list of instructions above it. We will give you a number cooresponding to the instruction you should follow.
- The code between [[SELECTED]] [[/SELECTED]] is the ONLY code that you should replace. So when you start responding, only respond with code that should replace it, and nothing else.
- MAKE SURE YOU ONLY WRITE CODE TO REPLACE WHATS BETWEEN [[SELECTED]] AND [[/SELECTED]]. The rest of it doesnt need to be replaced
- The rest of the code is ONLY provided as context. Sometimes, I'll collapse my code for you, and hide the details under '...'. Follow the style of the rest of the provided code.
- DO NOT WRITE THE ENTIRE CODE FILE. ONLY REPLACE WHAT IS NECESSARY IN SELECTED
...
${text}
NEW CODE TO REPLACE WHAT IS BETWEEN [[SELECTED]] and [/SELECTED]:
YOU ARE RESPONDING TO COMMENT NUMBER ${number}
`;
};

const reviewSuggestionsPrompt = async (
  text: string,
  filepath: string
): Promise<string> => {
  const defaultInstructions = `
- The text under 'CODE' has come straight from my text editor. 
- Your entire output MUST be comments to suggest how to improve the code
- Output these comments as a numbered list of suggestions to improve the code
- MAKE SURE YOU USE THE RIGHT COMMENT SYNTAX FOR THE LANGUAGE YOU ARE REVIEWING SUCH AS // FOR JAVASCRIPT OR # FOR PYTHON
- So starting a line with 1. would be wrong in javascript, but // 1. would be correct
- MAKE SURE YOU ONLY COMMENT ON WHATS BETWEEN [[SELECTED]] AND [[/SELECTED]]. The rest of it doesnt need suggestions
- The rest of the code is ONLY provided as context. Sometimes, I'll collapse my code for you, and hide the details under '...'. Follow the style of the rest of the provided code.
- DO NOT WRITE ANY CODE. ONLY COMMENT ON WHAT IS NECESSARY IN SELECTED
- Prefer functional programming principles.
`;
  const configurationInstructions = await workspace
    .getConfiguration()
...
CODE:
${text}
COMMENTS TO IMPROVE WHAT IS BETWEEN [[SELECTED]] and [/SELECTED]:
`;
};

export async function activate(context: ExtensionContext) {
  let cancellationTokenSource: any = undefined;
  const directedHealDisposable = commands.registerCommand(
    "wolverine.directedHeal",
    async () => {
      ...
      const contextText = visibleText.replace(
        selectedText,
        "[[SELECTED]]\n" + selectedText + "\n[[/SELECTED]]"
      );
      ...
      await useBufferToUpdateTextContentsWhileStreamingOpenAIResponse(
        activeEditor,
        range.start,
        prompt,
        cancellationTokenSource
  ...
  const directedReviewResponseHealDisposable = commands.registerCommand(
    "wolverine.directedReviewResponseHeal",
    async (number) => {
      commands.executeCommand("setContext", "wolverine.operationRunning", true);
      cancellationTokenSource = new CancellationTokenSource();
      ...
      const filePath = workspace.asRelativePath(document.uri);
      const prompt = await directedReviewResponseHealPrompt(
        contextText,
        filePath,
        number
      );
  ...
  const reviewDisposable = commands.registerCommand(
    "wolverine.review",
    async () => {
      commands.executeCommand("setContext", "wolverine.operationRunning", true);
      cancellationTokenSource = new CancellationTokenSource();
      ...
      const filePath = workspace.asRelativePath(document.uri);
      const prompt = await reviewSuggestionsPrompt(contextText, filePath);
  ...
  const cancelOpDisposable = commands.registerCommand(
    "wolverine.cancelOperation",
    () => {
      commands.executeCommand(
        "setContext",
        "wolverine.operationRunning",
  ...
  context.subscriptions.push(reviewDisposable, cancelOpDisposable);
  context.subscriptions.push(
    directedReviewResponseHealDisposable,
    cancelOpDisposable
  );
}
